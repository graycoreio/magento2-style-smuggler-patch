...
class ValidateUrlGeneratorClassTest extends TestCase
{
    ...
    public function testItRejectsAnythingThatIsNotARowUrlGenerator()
    {
        $rejected = [
            'arbitrary core class' => \Magento\Framework\DataObject::class,
            'the interface itself' => GeneratorInterface::class,
        ...
        foreach ($rejected as $description => $generatorClassName) {
            try {
                $this->plugin->beforeCreateUrlGenerator($this->subject, $generatorClassName, ['data' => 'x']);
                $this->fail($description . ' should have been rejected');
            } catch (\InvalidArgumentException $exception) {
                $this->assertSame('Passed wrong parameters', $exception->getMessage(), $description);
            }
        }
    }
}
